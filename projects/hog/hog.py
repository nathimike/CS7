...
partner_one_email = "nathimike102@gmail.com"
partner_two_email = ""
assignment = "hog"
...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN Question 1
    total_sum, pig_out_rule = 0, False
    while num_rolls:
        dice_value = dice()
        if dice_value == 1:
            pig_out_rule = True
        ...
        num_rolls -= 1
    return 1 if pig_out_rule else total_sum
    # END Question 1
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    x =[int(i) for i in str(opponent_score)]
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    return (opponent_score, score) if who else (score, opponent_score) # You may wish to change this line.
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    i = 1
    while i <= 10:
        max_score = make_averaged(roll_dice)(i, dice)
        print(i,"dice scores",max_score,"on average")
        num_dice = i
        i += 1
    return num_dice
...
def swap_strategy(score, opponent_score):
    ...
    5
    >>> swap_strategy(50, 80) # 50 + (1 + max(8, 0)) = 59: Lots of free bacon
    0
    ...
    if turn_score == (opponent_score // 2):
        return 0
    elif turn_score == (opponent_score * 2):
        return 5
    else:
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    If one can swap(Beneficial Swap) and swap difference is greater than 20 then roll 0.
    If one can use bacon strategy and swap strategy, make sure that after using will not cause a 
    self-harming swap then roll 0 otherwise roll 5.
    Otherwise change Baseline num rolls and bacon margin depending on the score, given that less num rolls with a higher score.
    If one can roll 0 and cause the opponent to roll a four sided die, roll 0. 
    Otherwise, roll 6.
    """

    turn_score =take_turn(0, opponent_score) + score
    if turn_score >= 100 and not swap_strategy(turn_score, opponent_score):
        return 0

    if swap_strategy(score, opponent_score) == 0 and (turn_score - opponent_score) > 20:
        return 0

    if bacon_strategy(score, opponent_score) == 0:
        if swap_strategy(turn_score, opponent_score):
            if opponent_score > turn_score:
                return 0
            else:
                return 5
    else:
        if score <= 50:
            BASELINE_NUM_ROLLS, BACON_MARGIN = 6, 9
        elif score <= 80:
            BASELINE_NUM_ROLLS, BACON_MARGIN = 5, 8
        elif score <= 90:
            BASELINE_NUM_ROLLS, BACON_MARGIN = 3, 6
        else:
            BASELINE_NUM_ROLLS, BACON_MARGIN = 0, 5
        return swap_strategy(score, opponent_score)

    if select_dice(turn_score, opponent_score) == four_sided:
        return 0
    return 6
